...
router.get('/compare-periods', async (req, res) => {
  try {
    ...
    if (!period1 || period1.length === 0 || !period2 || period2.length === 0) {
      return res.json({
        period1: { 
          dates: { from: fromDate1, to: toDate1 },
          summary: { earnings: 0, views: 0 },
          posts: []
        },
        period2: { 
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const monthSummary = monthDeltas && monthDeltas.length > 0 
      ? monthDeltas.reduce((acc, delta) => ({
